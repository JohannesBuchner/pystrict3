...
def __usage():
    print('usage: %s [options] imagefile ...' % sys.argv[0], file=sys.stderr)
...
def __main():
    ...
    if '-b' in optdic: generator.setbasesize(optdic['-b'])
    if '-o' in optdic: generator.setoutdir(optdic['-o'])
    if '-s' in optdic: generator.setsuffix(optdic['-s'])
