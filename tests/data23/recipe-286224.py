...
class HTMLLinkScanner(html.parser.HTMLParser):
  ...
  def handle_starttag(self, tag, attrs):
    if tag in self.tags:
      ...
      if isinstance(checkattrs, str):
        checkattrs = [checkattrs]
      for attr, value in attrs:
        if attr in checkattrs:
          if tag != 'base':
            link = urllib.parse.urldefrag(value)[0]
...
class MirrorRetriever:
  ...
  def url2filename(self, url):
    scheme, location, path, query, fragment = urllib.parse.urlsplit(url)
  ...
  def mirror(self, url):
    if url in self.urlmap:
      ...
      for link in linkscanner.links:
        linkurl = urllib.parse.urljoin(url, link)
      ...
      for tagtext, attr, link in linkscanner.replacements:
        scheme, location, path, query, fragment = urllib.parse.urlsplit(link)
        newtext = None
...
class WaybackRetriever(MirrorRetriever):
  ...
  def retrieveurl(self, url):
    ...
    contents = urllib.request.urlopen(waybackurl).read()
    if contents.find("Sorry, we can't find the archived version of this page") != -1:
      raise urllib.error.URLError("not in wayback archive")
