...
def tensor(sizes=0, elem=0):
    ...
    if sizes:
        first = sizes.pop()
        if isinstance(elem, (int, str, tuple, NoneType, bool)):
            result = [elem] * first
        else:
            result = [deepcopy(elem) for i in range(first)]
        while sizes:
            result = [deepcopy(result) for i in range(sizes.pop())]
    return result
...
def transpose(m):
    ...
    if isinstance(m, list):
        if isinstance(m[0], list):
            ...
            return list(zip(*m)) # faster
    else:
        if isinstance(m[0], list):
            return tuple(map(list, list(zip(*m))))
        else:
            return tuple( zip(*m) )
