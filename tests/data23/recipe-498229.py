...
def wchoice(objects, frequences, filter=True, normalize=True):
    ...
    if filter:
        ...
        frequences = list(map(float, frequences))
        filteredFreq = []
        filteredObj = []
        for freq, obj in zip(frequences, objects):
            if freq < 0:
...
if __name__ == '__main__':
    ...
    wc = wchoice(["a"], [1])
    assert set(wc() for i in range(20000)) == set(["a"])

    wc = wchoice(["a"], [0])
    assert set(wc() for i in range(20000)) == set([None])
    ...
    assert [wc() for _ in range(10)] == ["A"] * 10

    objs = "ABCDE"
    freqs = [1, 3, 1.1, 0.1, 5]
    wc = wchoice(objs, freqs, filter=False)
    print([wc() for _ in range(50)])
