...
def RoadRunner(): 
	...
	file_extension = input ( 'Enter extension of the files to be updated [.html] : ')
	if not file_extension: file_extension = '.html' 

	while 1: 
		regexp = input ( 'Enter regex string [e.g: <[Bb][oO][dD][yY].*>] : ' ) 
		if regexp: break 

	while 1: 
		update_path = input ( 'Enter path to update [%s] : ' % (os.getcwd()) ) 
