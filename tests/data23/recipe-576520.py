...
import pickle as pickle # Only for SimpleChannelEndpoint
...
class Mux(ReceiverThread):
    ...
    def run(self):
        ...
        self.__lock.acquire()
        try:
            for xid, slot in self.__waitq.items():
...
def _test():
    ...
    class MyDeMux(DeMux):
        ...
        def process_transaction(self, message_before, duration, message_after):
            ...
            print("[%d] BEGIN: %s (sleep %fs)" % (_thread.get_ident(),
                                                  message_before, duration))
            time.sleep(duration)
            print("[%d] END: %s" % (_thread.get_ident(), message_after))
