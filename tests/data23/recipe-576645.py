		...
		for group in self.Lgroups:
			for locus in group:
				...
				if self.randmut.random() < self.MutationProbability:
					al = self.randsel.choice(list(locus.Alleles.keys()))
				# select allele by it`s frequency in genotype
				else:
					while al == None:
						for allele in list(locus.Alleles.keys()):
...
if __name__ == "__main__":
	...
	print('--------------------------------------------')
	gen = sgaGenotype([list(range(2,61)),list(range(2,61)),list(range(2,61))], 1, testFitness, testNotify, Minimize = True)
	gen.Evolve(1000)
