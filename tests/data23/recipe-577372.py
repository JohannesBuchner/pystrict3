...
class Attributes(object):
    ...
    def __repr__(self):
        return "%s(%s)" % (self.__class__.__name__, ", ".join([
            "%s = %r" % v for v in filter(lambda name__1: name__1[0][0] != '_',
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        if self._entries == 1:
            callers = {}
            for s in self._callers: callers.update(list(zip(list(s.keys()), [s] * len(s))))
            currentNames = set(callers.keys())
            savedNames = set()
            for s in self._backups: savedNames.update(list(s.keys()))
...
class Scope(Attributes):
    ...
    def __repr__(self):
        return "%s(%s)" % (self.__class__.__name__, ", ".join(
            ...
            + ["%s = %r" % v for v in filter(lambda name__: name__[0][0] != '_',
                                              iter(self.__dict__.items()))]))
...
class Workspace(Attributes):
    ...
    def __repr__(self):
        ...
        return "%s(%s)" % (self.__class__.__name__, ", ".join(
            [repr(self._name)] +
            ["%s = %r" % v for v in [(a, getattr(self, a)) for a in filter(
