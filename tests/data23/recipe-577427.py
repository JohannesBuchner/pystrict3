...
if n < 2:
    sys.exit()
m = int(input('Number of constraints for each item: '))
if m < 2:
    sys.exit()
...
for j in range(m):
    constraintName = input('Constraint ' + str(j + 1) + ' name: ')
    constraintNames.append(constraintName)
    constraintType = int(input('Lower(0) or Higher(1) is better: '))
...
constraintsTable = []
for i in range(n):
    itemName = input('Item ' + str(i + 1) + ' name: ')
    itemNames.append(itemName)

    constraints = []
    for j in range(m):
        constraint = float(input(constraintNames[j] + ': '))
        if constraint == 0.0:
