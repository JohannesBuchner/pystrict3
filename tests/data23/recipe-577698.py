...
class pynumgrid:
	...
	def select( self, query, ns={} ):
		...
		ns.update( dict( _en=enumerate, _rc=range(self.rowcount),
			array=array.array ) )
...
if __name__ == '__main__':
	...
	blocks = 10
	if '-i' in sys.argv:
		agelist = [ (i*1779+517)%80+18 for i in range(blocksize) ]
		heightlist = [ (i*5111+711)%90+120 for i in range(blocksize) ]
		weightlist = [ (i*3113+797)%100+50 for i in range(blocksize) ]
	...
	if '-q' in sys.argv:
		print('The maximum height (in cm) of people in their 30s:', end=' ')
		print(pyg.select('max(height[i] for i in _rc if 29<age[i]<40)'))
		print(pyg.fetchlist( list(range(100,110)), cols=['weight','height'] ))
