...
def cheaper_date(dt_str, kw_list = [], reg_xp = r'\D', val_conv = int):
    ...
    try:
        tm_list = list(map(val_conv, re.split(reg_xp, dt_str)))
    ...
    if kw_list:
        tm_list = dict(list(zip(kw_list, tm_list)))
