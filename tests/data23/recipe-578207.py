...
def which_CD(maximum):
    ...
    while True:
        try:
            num = int(input('What cdrom drive do you want to use? '))
        except ValueError:
...
def cmd(disc):
    ...
    table = list(map(disc.get_track_audio, list(range(total))))
    if sum(table) == 0:
...
def get_track(maximum):
    while True:
        try:
            num = int(input('What track should be played? '))
        except ValueError:
            print('Please try typing a number.')
        except:
            return -1
