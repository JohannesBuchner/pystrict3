...
def get_creatures():
    while True:
        try:
            creatures = int(input('How many creatures do you want to start with? '))
            if creatures > 0:
                return creatures
...
def get_years():
    while True:
        try:
            years = int(input('How many years do you want simulated? '))
            if years > 0:
                return years
...
def get_summary():
    while True:
        try:
            answer = input('Do you want a summary of the simulation? ').lower()
            if answer == 'yes':
                return True
            if answer == 'no':
                return False
...
def get_target():
    while True:
        try:
            target = int(input('What is the target population (0 for None)? '))
            if target > 0:
                return target
            return 0
        except:
...
def get_exit():
    while True:
        try:
            answer = input('Do you wish to exit this program? ').lower()
            if answer == 'yes':
                return True
            if answer == 'no':
                return False
