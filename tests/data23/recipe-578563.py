...
class TTTGame(object):
    ...
    def __init__(self, game=None):
        if not game:
            self.board = list(map(list, ['___'] * 3))
            self.avail = set(range(1, 10))
            self.xnums = set()
            self.onums = set()
        else:
            self.board = list(map(list, game.board))
    ...
    def __str__(self):
        ...
        divs = ['\n---+---+---\n', '\n---+---+---\n', '\n']
        return ' ' + ' '.join(map(''.join, list(zip(rows, divs))))
    ...
    def next_move(self, piece):
        ...
        """
        if all([len(set(x)) == 1 for x in self.board]):
...
class CLI(object):
    ...
    def coin_toss(self):
        ...
        while True: # until user enters valid input
            self.refresh()
            option = input("Heads or Tails (or just hit enter)? ")
    ...
    def player_turn(self):
        while True: # until user enters valid input
            self.refresh()
            option = input("cell number: ").strip().lower()
            if option == 'hint':
    ...
    def play_again(self):
        ...
        while True: # until user enters valid input
            self.refresh()
            option = input("Play again (enter) or n? ").strip().lower()
            if not option:
                self.message = ''
                return
            elif option in ["no", 'n']:
