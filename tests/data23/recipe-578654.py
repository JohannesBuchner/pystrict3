import arcpy as ap, os, sys, time, tkinter as Tk, tkinter.filedialog
...
def SelectFolder(req = 'Please select a fgdb:'):
    ...
    try:
        root = Tk.Tk(); root.withdraw(); mypath = tkinter.filedialog.askdirectory(initialdir="C:\\", title=req); root.destroy()
        os.chdir(mypath)
        return mypath
    except:
...
# Set workspace
gdb_type = input('Do you want to open a fgdb or dbf (f/d): ')
...
if gdb_type == 'd':
    ...
    for dataset in ap.ListDatasets():
        ...
        for fc in ap.ListFeatureClasses():
            ...
            if numrec > 0:  print(' -FC: {0}: {1} features'.format(fc, numrec))
    print('This arcpy cataloging took {} sec.'.format(str(time.clock()-tic)[:8]))
yorn = input('Copy the lines above and paste into your document/log-file before pressing a key to quit...')
