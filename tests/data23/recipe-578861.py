...
env = input("Please enter your test env: qacand, qacandrot or qapatch (Non case sensitive): ")
...
while 1:
    build_by_module = input("Is your module build by module: (y or n, Non case sensitive) ")

    if build_by_module.strip().lower() == "n":
        your_module = "V4"
        your_svn_number = input("Please enter your svn number (6 digit): ")
        ...
        your_module = input("Please enter your module name: ")
        your_svn_number = input("Please enter your svn number (6 digit): ")
        if your_module.strip().lower() in list(module_svn_map.keys()):
            if int(your_svn_number) <= int(module_svn_map[your_module]):
                print("Your svn number is included in current build on " + env.strip().lower() + ".")
            else:
                print("Your svn number is NOT included on " + env.strip().lower() + ".")
