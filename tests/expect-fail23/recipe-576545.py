...
choice=None
def AUDPC():
      num_ob=eval(input('Enter the number of observations: '))
      di=[0]*num_ob
      tim=[0]*num_ob
      index=0
      while index<num_ob:
                  di[index]=eval(input('Enter the value of the Index of Makkiney: '))
                  index+=1
      index1=0
      while index1<num_ob:
                 tim[index1]=eval(input('Enter the time of the observation in days: '))
                 index1=index1+1
                 meanvec=[0]*(num_ob-1)
                 intvec=[0]*(num_ob-1)
